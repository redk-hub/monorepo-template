# Monorepo 前端项目

一个基于 pnpm workspace 的现代化前端 monorepo 项目，采用模块化架构设计，包含主 Web 应用和多个可复用包。
![alt text](/preview/image.png)
![alt text](/preview/image2.png)

## 🚀 项目优势

- **节省磁盘空间**：共享依赖，减少重复安装
- **统一依赖版本**：确保所有包使用相同版本的依赖
- **跨包协作与复用**：组件、工具、hooks 可在不同应用间共享
- **提升构建效率**：增量构建和缓存优化

## 📁 项目结构

```
my-repo/
├── apps/
│   └── web/                 # 主 Web 应用 (Vite + React)
│       ├── src/
│       │   ├── pages/       # 页面组件
│       │   ├── router/      # 路由配置
│       │   ├── Layout/      # 布局组件
│       │   └── mocks/       # 模拟数据
│       ├── package.json
│       └── vite.config.js
├── packages/
│   ├── hooks/              # 公共 Hooks
│   ├── pc-ui/             # PC 端组件库
│   ├── mobile-ui/         # 移动端组件库
│   └── utils/             # 工具函数
├── package.json
└── pnpm-workspace.yaml
```

## 📦 包职责说明

### apps/web

主站应用，基于 Vite + React 构建，负责：

- 页面路由和导航
- UI 组件组装
- 与后端 API 通信
- 用户界面渲染

### packages/hooks

可复用 React Hooks：

- `usePermission` - 权限管理
- `useUserStore` - 用户状态管理
- `useCurMenu` - 当前菜单状态

### packages/pc-ui & packages/mobile-ui

响应式 UI 组件库：

- 通用 UI 组件
- 适配不同设备的组件版本
- 统一的样式规范

### packages/utils

工具函数库：

- HTTP 请求封装
- 日期处理工具
- 通用工具方法

## 🛠️ 技术栈

### 核心框架

- **React 18** - 用户界面构建
- **Vite** - 构建工具和开发服务器
- **React Router** - 客户端路由
- **zustand** - 状态管理
- **React Query** - 数据获取和状态管理

### 数据管理

- **zustand** - 轻量级状态管理,持久化支持,存储客户端操作状态
- **React Query (TanStack Query)** - 强大的数据同步和缓存管理，处理服务端数据
- **axios-mock-adapter** - HTTP 请求模拟和测试工具

### 开发工具

- **pnpm** - 包管理器 (workspace 模式)
- **ESLint** - 代码质量检查
- **Prettier** - 代码格式化

## 🏃‍♂️ 快速开始

### 环境要求

- Node.js 22+
- pnpm 7+

### 安装依赖

```bash
# 在项目根目录执行
pnpm install
```

### 启动开发服务器

```bash
# 启动 web 应用
pnpm --filter apps/web start:dev

# 或者进入 apps/web 目录运行
cd apps/web
pnpm start:dev
```

### 构建和测试

```bash
# 代码检查
pnpm lint

# 代码格式化
pnpm format
```

## 🎯 核心功能示例

### React Query 数据管理

项目深度集成 React Query (TanStack Query) 进行数据管理，提供以下功能：

#### 基础查询 (Basic Query)

```jsx
// 使用 useQuery 进行数据获取
const { data, isLoading, isError } = useQuery({
  queryKey: ['users', { page: 1, size: 5 }],
  queryFn: fetchUsers,
});
```

#### 高级功能

- **分页查询** - 支持分页数据的缓存和同步
- **无限滚动** - 实现无限加载列表
- **依赖查询** - 基于其他查询结果的数据获取
- **并行查询** - 同时发起多个请求
- **数据变更** - 使用 useMutation 处理数据修改
- **预加载和失效** - 优化用户体验

#### 配置选项

```jsx
// queryClient.js 中的全局配置
const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      retry: 1,
      refetchOnWindowFocus: false,
      staleTime: 1000 * 30, // 30秒
      gcTime: 1000 * 60 * 5, // 5分钟
    },
  },
});
```

### Mock API 模拟

使用 `axios-mock-adapter` 进行 API 请求模拟，支持：

#### 请求拦截和响应模拟

```jsx
// mockApi.js 中的配置
const mock = new MockAdapter(request, { delayResponse: 500 });

// GET 请求模拟
mock.onGet('/api/users').reply((config) => {
  const params = config.params || {};
  const page = parseInt(params.page || 1, 10);
  const size = parseInt(params.size || 5, 10);
  // 返回模拟数据
  return [200, { code: 0, data: { list: [], total: 0 } }];
});
```

#### 支持的 HTTP 方法

- `GET` - 数据查询
- `POST` - 数据创建
- `PATCH` - 数据更新
- `DELETE` - 数据删除

#### 特性

- **延迟响应** - 模拟真实网络延迟
- **参数解析** - 支持 URL 参数和请求体解析
- **错误模拟** - 支持各种 HTTP 状态码和错误响应
- **动态数据** - 基于请求参数返回不同数据

### User 模块状态保持实现

在 `apps/web/src/pages/User/` 目录下提供了从列表页跳转到详情页并保持列表状态的完整实现示例。

#### 实现目标

从用户列表页跳转到详情页，返回时保持列表的：

- 分页状态
- 筛选条件
- 排序设置
- 滚动位置

#### 实现方案

**方案一：状态提升 (ListLayout)**

```jsx
// ListLayout.jsx - 父组件管理状态
const [listState, setListState] = useState({
  page: 1,
  pageSize: 10,
  filters: {},
  sort: {},
  scrollTop: 0,
});

// 传递给子组件
<List listState={listState} onStateChange={setListState} />;
```

**方案二：全局状态管理 (pages/react-query/Pagination.jsx)**

```jsx
// 记录页面参数
const [page, setPage] = useState(() => {
  return Number(sessionStorage.getItem('last_page')) || 1;
});

// 返回页面从 sessionStorage 中获取，react-query取对应key的列表数据
```

## 📚 开发指南

### 添加新页面

1. 在 `apps/web/src/pages/` 创建页面组件
2. 在 `apps/web/src/router/routeConfig.jsx` 配置路由
3. 使用 `packages/pc-ui` 或 `packages/mobile-ui` 中的组件

### 创建可复用包

1. 在 `packages/` 目录下创建新包
2. 配置包的 `package.json`
3. 在其他应用中通过 workspace 引用

### 代码规范

- 使用提供的 ESLint 和 Prettier 配置
- 遵循 React Hooks 最佳实践
- 组件和函数使用清晰的命名

## 🤝 贡献指南

欢迎提交 Issue 和 Pull Request 来改进项目！

---

**项目持续维护中，如有问题请查看 Issues 或提交新的 Issue。**
